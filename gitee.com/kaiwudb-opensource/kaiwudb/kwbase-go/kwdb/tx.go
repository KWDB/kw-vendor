...
// Package kwdb provides helpers for using CockroachDB in client
// applications.
package kwdb
...
func ExecuteInTx(ctx context.Context, tx Tx, fn func() error) (err error) {
	...
	// errors.
	if _, err = tx.ExecContext(ctx, "SAVEPOINT kwbase_restart"); err != nil {
		return err
	}
	...
	for {
		...
		if err == nil {
			...
			released = true
			if _, err = tx.ExecContext(ctx, "RELEASE SAVEPOINT kwbase_restart"); err == nil {
				return nil
			}
		}
		...
		if _, retryErr := tx.ExecContext(ctx, "ROLLBACK TO SAVEPOINT kwbase_restart"); retryErr != nil {
