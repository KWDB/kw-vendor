...
import (
	...
	"github.com/yuin/gopher-lua/golua-parse"
...
// ParseString is used to check udf grammar
func (ls *LState) ParseString(source string, udfName string, argLen int) error {
	chunk, err := golua_parse.Parse(strings.NewReader(source), "<string>")
	if err != nil {
		return newApiErrorE(ApiErrorSyntax, err)
	}

  if len(chunk) == 1 {
		if v1, ok := chunk[0].(*ast.FuncDefStmt); ok {
       if v2, ok := v1.Name.Func.(*ast.IdentExpr); ok {
				 if strings.ToLower(v2.Value) == udfName {
					if  len(v1.Func.ParList.Names) != argLen {
						return newApiErrorE(ApiErrorSyntax, fmt.Errorf("The number of %s function parameters does not match", udfName))
					}

					 hasOneReturn := checkRetStmts(v1.Func.Stmts)
					 if !hasOneReturn {
						return newApiErrorE(ApiErrorSyntax, fmt.Errorf("The return statement in the function body of %s can only contain one return value", udfName))
					 }

					 _, err = Compile(chunk, "<string>")
					 if err != nil {
						 return newApiErrorE(ApiErrorSyntax, err)
					 }
					 return nil
	...
	return newApiErrorE(ApiErrorSyntax, fmt.Errorf("The function body must contain only one function named %s", udfName))
}

func checkRetStmts(stmts []ast.Stmt) bool {
	for _, stmt := range stmts {
		...
		case *ast.IfStmt:
			if !checkRetStmts(s.Then) || (s.Else != nil && !checkRetStmts(s.Else)) {
				return false
			}
		case *ast.WhileStmt:
			if !checkRetStmts(s.Stmts) {
				return false
			}
		}
	}
